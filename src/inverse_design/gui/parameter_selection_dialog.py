...
class ParameterSelectionDialog:
    ...
    def create_parameter_checkboxes(self, param_names):
        ...
        for param in param_names:
            ...
            ttk.Checkbutton(
                self.scrollable_frame, text=param, variable=self.param_vars[param]
    ...
    def create_buttons(self):
        ...
        ttk.Button(left_buttons, text="Select All", command=self.select_all).pack(
            side="left", padx=5
        )
        ttk.Button(left_buttons, text="Deselect All", command=self.deselect_all).pack(
            side="left", padx=5
        )

        ttk.Button(button_frame, text="Create Visualization", command=self.create_vis).pack(
            side="right", padx=5
        )
    ...
    def create_vis(self):
        ...
        try:
            ...
            VisualizationWindow(
                self.dialog, parameter_pdfs, self.abc_gui.abc_config, selected_params
            )
