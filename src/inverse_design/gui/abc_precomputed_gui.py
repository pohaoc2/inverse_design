...
class ABCPrecomputedGUI:
    ...
    def setup_gui(self):
        ...
        ttk.Entry(file_frame, textvariable=self.config_file_path, width=50).grid(
            row=0, column=1, padx=5
        )
        ttk.Button(file_frame, text="Browse", command=lambda: self.browse_file("config")).grid(
            row=0, column=2
        )

        ttk.Label(file_frame, text="Parameter File:").grid(row=1, column=0, sticky="w")
        ttk.Entry(file_frame, textvariable=self.param_file_path, width=50).grid(
            row=1, column=1, padx=5
        )
        ttk.Button(file_frame, text="Browse", command=lambda: self.browse_file("param")).grid(
            row=1, column=2
        )

        ttk.Label(file_frame, text="Metrics File:").grid(row=2, column=0, sticky="w")
        ttk.Entry(file_frame, textvariable=self.metrics_file_path, width=50).grid(
            row=2, column=1, padx=5
        )
        ttk.Button(file_frame, text="Browse", command=lambda: self.browse_file("metrics")).grid(
            row=2, column=2
        )
        ...
        ttk.Radiobutton(
            ...
            value="BDM",
            command=self.update_available_metrics,
        ...
        ttk.Button(
            button_frame, text="Create Visualization", command=self.create_visualization
        ).grid(row=0, column=1, padx=5)
        ttk.Button(button_frame, text="Metrics KDE", command=self.show_metrics_kde).grid(
    ...
    def setup_target_options(self):
        ...
        for i, metric in enumerate(available_metrics):
            ...
            ttk.Checkbutton(
                self.targets_frame, text=metric.value, variable=self.target_vars[metric]
    ...
    def run_abc(self):
        ...
        try:
            ...
            average_distance = (
                sum(result["distance"] for result in accepted_samples) / accepted_count
                if accepted_count > 0
                else 0
            )

            self.status_label.config(
                text=f"ABC inference completed successfully!\nAccepted samples: {accepted_count}/{abc.num_samples}, Avg Distance: {average_distance:.2f}"
            )
    ...
    def show_metrics_kde(self):
        ...
        try:
            ...
            target_values = {
                target.metric: target.value for target in selected_targets
            }  # Store target values

            # Create visualization window
            MetricsVisualizationWindow(
                self.root, metrics_df, accepted_indices, metric_names, target_values
            )
