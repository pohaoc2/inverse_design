...
def evaluate_parameters(abc_instance, parameters: Dict[str, float]) -> Tuple[Dict, float]:
    ...
    metrics = abc_instance.calculate_metrics(grid_states, time_points)
    distance = float(abc_instance.calculate_distance(metrics))
    return {
        'metrics': metrics,
        'distance': distance
    }

def compare_sampling_methods(abc_instance, pdf_results: Dict, num_samples: int = 100) -> Dict[str, Dict]:
    ...
    # 1. Use mean values from independent distributions
    independent_means = {}
    ...
    # 2. Sample from joint distribution
    joint_samples = np.random.multivariate_normal(
        pdf_results['joint_mean'],
        pdf_results['joint_cov'],
        size=num_samples
    )
